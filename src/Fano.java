...
public class Fano {
     ...
     */
    public void sedonionFanoCombinationUtil(int[] arr, int[] data, int start,
                                    int end, int index, int r, int[][] masterData, int[] mdIndex, int n, int[][] combs) {
        if (index == r) {
            int[][] tripletOctonions = new int[32][32];
            for (int spot = 0; spot < 105; spot++) {
                for (int begin = 0; begin < 3; begin++) {
                    ...
                    tripletOctonions[combs[data[spot]][b] + 1][combs[data[spot]][a] + 1] = (combs[data[spot]][c] + 1 + 16) % 32;
                }
            }
            for (int row = 0; row < 32; row++) {
                tripletOctonions[row][0] = row;
                tripletOctonions[0][row] = row;
                tripletOctonions[row][row] = 16;
            }
            tripletOctonions[0][0] = 0;
            tripletOctonions[16][16] = 0;
            for (int row = 0; row < 32; row++) {
                for (int column = 0; column < 32; column++) {
                    if (row == column || tripletOctonions[row][column] == 0) {
                        continue;
                    }
                    tripletOctonions[(row + 16) % 32][(column + 16) % 32] = tripletOctonions[row][column];
                    tripletOctonions[(row) % 32][(column + 16) % 32] = (tripletOctonions[row][column] + 16) % 32;
                    tripletOctonions[(row + 16) % 32][(column) % 32] = (tripletOctonions[row][column] + 16) % 32;
                }
            }
            if (isLatin(tripletOctonions)) {
                ...
                mdIndex[0]++;
                System.out.println("mdIndex[0]="+mdIndex[0]);
            }
        } else {
    ...
    /**
     * Computes the 30 sets of seven sets of triples that can number
     * the Fano plane without repeats or position conflicts. It is an adapted
     * version of the combinations() function of PermutationsFactoradic. calls
     * the recursive fanoCombinationsUtil()
     *
     * @param n       number of indexes in the alphabet, always 7
     * @param r       length of selection from n, always 3
     * @param combs   7 choose 3
     * @param mdIndex a counter to index the output table
     * @return the 30 sets of 7 triples that satisfy the Fano plane mnemonic
     */
    public int[][] sedonionFanoCombinations(int n, int r, int[][] combs, int[] mdIndex) {
        ...
        int[][] masterData = new int[100000][r];
        // Print all combination using temporary array 'data[]'
        fanoCombinationUtil(arr, data, 0, n, 0, r, masterData, mdIndex, n, combs);
        int[][] out = new int[mdIndex[0]][r];
        for (int triplet = 0; triplet < mdIndex[0]; triplet++){
            for (int spot = 0; spot < r; spot++){
                out[triplet][spot] = masterData[triplet][spot];
            }
        }
        return out;
    }
     ...
     * @return array of 480 valid octonion multiplication tables
     */
    public int[][][] sedonionFanoGenerate() {
        //results will be placed in this for output
        int[][][] out = new int[100000][32][32];
        //results are placed here
        int[][][] tripletOctonions = new int[100000][32][32];
        //7 choose 3, there are 35 triplets out of 7
        int[][] combs = pf.combinations(15, 3);
        int[][] tripletIndexes = sedonionFanoCombinations(combs.length,105,combs,new int[1]);
        //precomputed results to save time, these are all the sets of 7 triplets from above that don't share more than one value
        //int[][] tripletIndexes = new int[][]{{0, 9, 14, 20, 23, 27, 28}, {0, 9, 14, 21, 22, 26, 29}, {0, 10, 13, 19, 24, 27, 28}, {0, 10, 13, 21, 22, 25, 30}, {0, 11, 12, 19, 24, 26, 29}, {0, 11, 12, 20, 23, 25, 30}, {1, 6, 14, 17, 23, 27, 31}, {1, 6, 14, 18, 22, 26, 32}, {1, 7, 13, 16, 24, 27, 31}, {1, 7, 13, 18, 22, 25, 33}, {1, 8, 12, 16, 24, 26, 32}, {1, 8, 12, 17, 23, 25, 33}, {2, 5, 14, 17, 21, 29, 31}, {2, 5, 14, 18, 20, 28, 32}, {2, 7, 11, 15, 24, 29, 31}, {2, 7, 11, 18, 20, 25, 34}, {2, 8, 10, 15, 24, 28, 32}, {2, 8, 10, 17, 21, 25, 34}, {3, 5, 13, 16, 21, 30, 31}, {3, 5, 13, 18, 19, 28, 33}, {3, 6, 11, 15, 23, 30, 31}, {3, 6, 11, 18, 19, 26, 34}, {3, 8, 9, 15, 23, 28, 33}, {3, 8, 9, 16, 21, 26, 34}, {4, 5, 12, 16, 20, 30, 32}, {4, 5, 12, 17, 19, 29, 33}, {4, 6, 10, 15, 22, 30, 32}, {4, 6, 10, 17, 19, 27, 34}, {4, 7, 9, 15, 22, 29, 33}, {4, 7, 9, 16, 20, 27, 34}};
        //precomputed triplets processed into Fano plane form
        ;
        int numTriplets = tripletIndexes.length;
        for (int triplet = 0; triplet < numTriplets; triplet++){
            System.out.print("fanoTriplets["+triplet+"]={");
            for (int spot = 0; spot < 105; spot++){
                System.out.print(tripletIndexes[triplet]);
                if (spot != 104) System.out.print(",");

            }
            System.out.print("};\n");

        }
        int[][][] fanoTriplets = new int[numTriplets][105][3];

        for (int set = 0; set < numTriplets; set++) {
            for (int spot = 0; spot < 105; spot++) {
                for (int sspot = 0; sspot < 3; sspot++) {
                    fanoTriplets[set][spot][sspot] = combs[tripletIndexes[set][spot]][sspot] + 1;
                }
        ...
        int index = 0;
        //current set of triplets being negative sign bitmasked and considered as potential octonions
        int[][] activeTriplets = new int[105][3];
        for (int set = 0; set < numTriplets; set++) {
            //bitmask loop
            //the seven bits of negs are applied as a negative sign bitmask to the values of activeTriplets[]
            //I've seen others on the internet reverse the triplets, but I've gotten it to work by applying
