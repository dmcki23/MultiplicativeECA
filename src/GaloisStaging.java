...
public class GaloisStaging {
    ...
    public boolean mols(int[][][] cube) {
        ...
        int[] flattened = new int[size * size * size];
        for (int row = 0; row < cube.length; row++) {
            for (int col = 0; col < cube[row].length; col++) {
                for (int zee = 0; zee < cube[row][col].length; zee++) {
                    int tot = size * size * row + size * col + zee;
                    flattened[tot] = cube[row][col][zee];
                }
            }
        }
        int totValid = 0;
